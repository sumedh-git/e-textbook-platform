...
function ModifyChapter() {
    ...
    const checkChapterExists = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/admin/check-chapter/${eTextbookID}/${chapterID}`);
    ...
    const handleOption = async (option) => {
        ...
        if (option === 1) {
            navigate('/admin/add-section', { state: { eTextbookID, chapterID } });
        } else if (option === 2) {
            navigate('/admin/modify-section', { state: { eTextbookID, chapterID } });
        } else if (option === 3) {
            navigate(-1);
        } else if (option === 4) {
