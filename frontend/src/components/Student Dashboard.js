    ...
    <div style={{ marginTop: '20px', padding: '20px', border: '1px solid #ccc' }}>
          {eTextbooks.map(etextbook => (
            <div style={{ borderBottom: '1px solid #ddd', marginBottom: '10px' }}>
              <h2> TextbookID:<b><i>{etextbook.ETextbookID}</i></b> Title: <b><i>{etextbook.Title}</i></b></h2>
              {etextbook.Chapters.map(chapter => (
                <div style={{ marginLeft: '70px', borderBottom: '1px solid #eee' }}>
                  <h3>ChapterID: {chapter.ChapterID} Title:{chapter.Title}</h3>
                  {chapter.Sections.map(section => (
                    <div style={{ marginLeft: '100px' }}>
                      <h4> SectionID:{section.SectionID} Title:{section.Title}</h4>
                    </div>
                  ))}
                </div>
              ))}
