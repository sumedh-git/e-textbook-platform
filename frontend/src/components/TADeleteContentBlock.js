  ...
  const handleSave = async () => {
    try {
      const response = await fetch(
        "http://localhost:5000/api/faculty/delete-content-block",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        setErrorMessage(
          errorData.error || "Failed to delete the content block."
        );
      }
    } catch (error) {
...
export default TADeleteContentBlock;
